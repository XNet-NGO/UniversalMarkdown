package io.noties.markwon.ext.tables;

import android.text.Spanned;
import android.text.TextUtils;
import android.view.View;
import android.widget.TextView;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;

import java.util.LinkedHashMap;
import java.util.Map;

abstract class TableRowsScheduler {

    static void schedule(@NonNull final TextView view) {
        final Object[] spans = extract(view);
        if (spans != null && spans.length > 0) {

            if (view.getTag(R.id.markwon_tables_scheduler) == null) {
                final View.OnAttachStateChangeListener listener = new View.OnAttachStateChangeListener() {
                    @Override
                    public void onViewAttachedToWindow(View v) {}
                    @Override
                    public void onViewDetachedFromWindow(View v) {
                        unschedule(view);
                        view.removeOnAttachStateChangeListener(this);
                        view.setTag(R.id.markwon_tables_scheduler, null);
                    }
                };
                view.addOnAttachStateChangeListener(listener);
                view.setTag(R.id.markwon_tables_scheduler, listener);
            }

            final TableRowSpan.Invalidator invalidator = new TableRowSpan.Invalidator() {
                final Runnable runnable = () -> view.setText(view.getText());
                @Override
                public void invalidate() {
                    view.removeCallbacks(runnable);
                    view.post(runnable);
                }
            };

            // Assign shared scroll states per table
            final Spanned spanned = (Spanned) view.getText();
            final Map<Integer, TableScrollState> tableStates = new LinkedHashMap<>();

            for (Object span : spans) {
                TableRowSpan row = (TableRowSpan) span;
                row.invalidator(invalidator);

                int start = spanned.getSpanStart(row);
                TableSpan[] tableSpans = spanned.getSpans(start, start + 1, TableSpan.class);
                int tableStart = tableSpans != null && tableSpans.length > 0
                        ? spanned.getSpanStart(tableSpans[0]) : start;

                TableScrollState state = tableStates.get(tableStart);
                if (state == null) {
                    state = new TableScrollState();
                    tableStates.put(tableStart, state);
                }
                row.setScrollState(state);
            }
        }
    }

    static void unschedule(@NonNull TextView view) {
        final Object[] spans = extract(view);
        if (spans != null && spans.length > 0) {
            for (Object span : spans) {
                ((TableRowSpan) span).invalidator(null);
            }
        }
    }

    @Nullable
    private static Object[] extract(@NonNull TextView view) {
        final Object[] out;
        final CharSequence text = view.getText();
        if (!TextUtils.isEmpty(text) && text instanceof Spanned) {
            out = ((Spanned) text).getSpans(0, text.length(), TableRowSpan.class);
        } else {
            out = null;
        }
        return out;
    }

    private TableRowsScheduler() {}
}
