...
public class PrinterMarkDownTextView extends AppCompatTextView implements IMarkdownLayer, DefaultLifecycleObserver, TextWatcher {
    ...
    public void setMarkdownText(@NonNull String markdown) {
        ...
        // Replace Unicode chars with ASCII fallbacks only if the font lacks the glyph
        markdown = sanitizeUnicode(markdown);
    ...
    /**
     * Implements the gradient typing animation
     */
    ...
    private static final char[][] UNICODE_FALLBACKS = {
            {'\u2192', ' ', '-', '>', ' '},
            {'\u2190', ' ', '<', '-', ' '},
            {'\u2194', ' ', '<', '-', '>', ' '},
            {'\u21D2', ' ', '=', '>', ' '},
    ...
    private Paint mGlyphCheckPaint;

    private String sanitizeUnicode(String s) {
        if (mGlyphCheckPaint == null) {
            mGlyphCheckPaint = getPaint() != null ? getPaint() : new Paint();
        }
        StringBuilder sb = null;
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i);
            char[] replacement = null;
            for (char[] entry : UNICODE_FALLBACKS) {
                if (c == entry[0]) {
                    if (!mGlyphCheckPaint.hasGlyph(String.valueOf(c))) {
                        replacement = entry;
                    }
                    break;
                }
            ...
            if (replacement != null) {
                ...
                sb.append(replacement, 1, replacement.length - 1);
            } else if (sb != null) {
                sb.append(c);
            }
        }
